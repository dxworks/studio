...
import {
  ...
  DXW_STUDIO_PROJECT_METADATA_FILE,
  studioConfigDefaultsFile,
  studioProjectsFile,
} from '../constants'
...
export const studioInit = new Command()
  ...
  .option('-ct, --chronos-tag <chronosTag>', 'the chronos docker tag', getDefaultValue('chronosTag'))
  .option('-cp, --chronos-port <chronosPort>', 'the chronos port')

  .option('-c1t, --chronos1-tag <chronos1Tag>', 'the chronos1 docker tag', getDefaultValue('chronos1Tag'))
  .option('-c1p, --chronos1-port <chronos1Port>', 'the chronos1 port')

  .option('-it, --illustry-tag <illustryTag>', 'the illustry docker tag', getDefaultValue('illustryTag'))
  .option('-ip, --illustry-port <illustryPort>', 'the illustry port')

  .option('-cr, --chronos-docker-registry <chronosDockerRegistry>', 'the link to the docker registry where the chronos images exist', getDefaultValue('chronosDockerRegistry'))
  .option('--network <network>', 'the docker network on which the containers should exist', getDefaultValue('network'))
  .option('-f, --force', 'whether to force the project creation by overwriting the existing folder', false)
  .option('--domain <domain>', 'the domain for traefik to deploy to', getDefaultValue('domain'))
  ...
  try {
    ...
    fs.writeFileSync(path.resolve(projectFolder, DXW_STUDIO_PROJECT_METADATA_FILE), JSON.stringify({
      ...
      chronos: {
        ...
        traefikUrl: `https://chronos.${projectID}.${options.domain}`,
        version: chronosVersion,
      },
      chronos1: {
        portUrl: `https://localhost:${options.chronos1Port}`,
        traefikUrl: `https://chronos1.${projectID}.${options.domain}`,
        version: chronos1Version,
      },
      illustry: {
        portUrl: `https://localhost:${options.illustryPort}`,
        traefikUrl: `https://illustry.${projectID}.${options.domain}`,
        version: illustryVersion,
      },
      composeFile: path.resolve(projectFolder, 'studio', 'docker-compose.yml'),
    }, null, 2))
...
function writeDockerCompose(studioFolder: string, options: StudioInitOptions, projectID: string) {
  ...
  if(!fs.existsSync(dockerComposeYmlTemplate)) {
    log.info(`Writing default docker-compose template to ${dockerComposeYmlTemplate}`)
    fs.copyFileSync(getAssetFile(dockerComposeAsset), dockerComposeYmlTemplate)
  }
  if (!fs.existsSync(studioConfigDefaultsFile)) {
    log.info(`Writing default config values to ${studioConfigDefaultsFile}`)
    const config = YAML.parse(fs.readFileSync(getAssetFile('config/defaults.yml')).toString())
    config.chronosDockerRegistry = options.chronosDockerRegistry
    fs.writeFileSync(studioConfigDefaultsFile, YAML.stringify(config))
  }

  const mongoInitAsset = 'init/mongo-init.js'
  const dockerComposeTemplateString = fs.readFileSync(dockerComposeYmlTemplate, 'utf-8')
...
async function fillOptions(options: StudioInitOptions) {
  ...
  if (!options.chronosDockerRegistry) {
    log.info('Chronos docker registry not found')
